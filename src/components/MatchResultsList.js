...
const MatchSummary = ({ match }) => {
    ...
    const getTeamLogo = (teamName) => {
        switch (teamName) {
            case 'FC Escuela':
                return escuelaLogo;
            case 'Barcelona Academy':
                return barcelonaLogo;
            case 'Valencia Academy':
                return valenciaLogo;
            case 'Ajax Youth':
                return ajaxLogo;
            default:
                return `https://via.placeholder.com/48/1e3c72/ffffff?text=${teamName.charAt(0)}`;
        }
    };
                ...
                <div className="stats-group">
                    <div className="possession-bar">
                        <div className="possession-label">
                            <span className="possession-title">POSSESSION</span>
                        </div>
                        <div className="possession-wrapper">
                            <span className="possession-value-label home">{match.stats.possession.home}%</span>
                            <div className="possession-track">
                                <div 
                                    className="possession-value home"
                                    style={{ width: `${match.stats.possession.home}%` }}
                                />
                                <div 
                                    className="possession-value away"
                                    style={{ width: `${match.stats.possession.away}%` }}
                                />
                            </div>
                            <span className="possession-value-label away">{match.stats.possession.away}%</span>
                        </div>
                    </div>
                    <div className="key-stats">
                        <div className="stat-item">
                            <span>Shots</span>
                            <strong>{match.stats.shots.home} - {match.stats.shots.away}</strong>
                        </div>
                        <div className="stat-item">
                            <span>On Target</span>
                            <strong>{match.stats.shotsOnTarget.home} - {match.stats.shotsOnTarget.away}</strong>
                        </div>
                    </div>
                </div>
                <Link to={`/matches/results/${match.id}`} className="match-report-link">
                    Full Report <FaChevronRight />
                </Link>
            </div>
        </div>
    );
...
const MatchResultsList = () => {
    ...
    const filteredMatches = selectedCompetition === 'all'
        ? matchesData
        : matchesData.filter(match => match.competition === selectedCompetition);
